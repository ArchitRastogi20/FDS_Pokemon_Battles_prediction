...
def load_data():
    ...
    df = pd.read_csv('submission_2/model_config_and_feature_selection/train_features.csv')
    ...
    print(f"\n  Data quality checks:")
    print(f"    Null values: {null_counts} {'OK' if null_counts == 0 else 'NOT OK'}")
    print(f"    Inf values: {inf_counts} {'OK' if inf_counts == 0 else 'NOT OK'}")
...
def train_with_kfold(X_train, y_train, best_params, use_wandb):
    ...
    if fold_thresholds:
        ...
        with open('submission_2/model_config_and_feature_selection/best_threshold_cv.json', 'w') as f:
...
def analyze_feature_importance(feature_importance_list, feature_names, use_wandb):
    ...
    # Save to CSV
    importance_df.to_csv('submission_2/model_config_and_feature_selection/feature_importance.csv', index=False)
...
def save_artifacts(models, final_model, best_params, fold_scores, feature_names, use_wandb):
    ...
    if final_model is not None:
        final_model.save_model('submission_2/model_config_and_feature_selection/model_final.json')
        print(f"  Saved model_final.json")
    
    # Save best parameters
    with open('submission_2/model_config_and_feature_selection/best_params.json', 'w') as f:
        json.dump(best_params, f, indent=4)
    print(f"  Saved best_params.json")
    
    # Save fold scores
    fold_df = pd.DataFrame(fold_scores)
    fold_df.to_csv('submission_2/model_config_and_feature_selection/fold_scores.csv', index=False)
    print(f"  Saved fold_scores.csv")
    
    # Save feature names
    with open('submission_2/model_config_and_feature_selection/feature_names.json', 'w') as f:
        json.dump(feature_names, f, indent=4)
    print(f"  Saved feature_names.json")
    
    # Save training config
    with open('submission_2/model_config_and_feature_selection/training_config.json', 'w') as f:
