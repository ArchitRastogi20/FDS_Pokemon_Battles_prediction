...
# ============================================================================
# MAIN FUNCTION FOR NOTEBOOK IMPORT
# ============================================================================

def run_eda(train_path, n_jobs=17):
    """
    Run comprehensive exploratory data analysis on training data.
    ...
    print("="*70)
    print("POKEMON BATTLE PREDICTION - EXPLORATORY DATA ANALYSIS")
    print(f"Using {n_jobs} CPU cores")
    print("="*70)
    
    # Load training data
    print(f"\n[1/8] Loading training data from: {train_path}")
    train_data = []
    ...
    print("\n[3/8] Calculating battle length statistics...")
    battle_lengths = [len(b['battle_timeline']) for b in train_data]
    ...
    # Pokemon species analysis
    print("\n[4/8] Analyzing Pokemon species...")
    p1_leads = [b['p1_team_details'][0]['name'] for b in train_data]
    p2_leads = [b['p2_lead_details']['name'] for b in train_data]
    ...
    for pokemon, count in p1_lead_counter.most_common(10):
        print(f"  {pokemon:20} - {count:4} battles ({count/len(train_data)*100:.1f}%)")
    ...
    # Type analysis
    print("\n[5/8] Analyzing Pokemon types...")
    all_p1_types = []
    for battle in train_data:
        for pokemon in battle['p1_team_details']:
            all_p1_types.extend([t for t in pokemon['types'] if t != 'notype'])
    ...
    # Base stats analysis
    print("\n[6/8] Analyzing base stats...")
    win_stats = {'hp': [], 'atk': [], 'def': [], 'spa': [], 'spd': [], 'spe': []}
    loss_stats = {'hp': [], 'atk': [], 'def': [], 'spa': [], 'spd': [], 'spe': []}
    
    for battle in train_data:
        p1_team = battle['p1_team_details']
        stats_dict = win_stats if battle['player_won'] else loss_stats
        
        for stat_name in ['hp', 'atk', 'def', 'spa', 'spd', 'spe']:
            avg_stat = np.mean([p[f'base_{stat_name}'] for p in p1_team])
            stats_dict[stat_name].append(avg_stat)
    ...
    # Battle timeline patterns
    print("\n[8/8] Analyzing battle timeline patterns...")
    first_turn_moves = []
    for battle in train_data:
        if battle['battle_timeline']:
            turn1 = battle['battle_timeline'][0]
            if turn1.get('p1_move_details'):
                first_turn_moves.append({
                    'move': turn1['p1_move_details']['name'],
                    'category': turn1['p1_move_details']['category'],
                    'player_won': battle['player_won']
                })
    ...
    if first_turn_moves:
        ...
        for move, count in move_counter.most_common(10):
            ...
            print(f"  {move:25} - {count:4} uses (Win rate: {win_rate:.1f}%)")
...
# ============================================================================
# SCRIPT MODE - RUNS WHEN EXECUTED DIRECTLY
# ============================================================================
